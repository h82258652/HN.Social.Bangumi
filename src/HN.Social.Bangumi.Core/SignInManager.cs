...
namespace HN.Social.Bangumi
{
    internal class SignInManager
    {
        ...
        internal async Task SignInAsync()
        {
            ...
            var requestTime = DateTime.Now;
            var authorizationUri = new Uri($"{Constants.AuthUrl}?client_id={_bangumiOptions.AppID}&response_type=code&redirect_uri={WebUtility.UrlEncode(_bangumiOptions.CallbackUrl)}");
            var authorizationResult = await _authorizationProvider.AuthorizeAsync(authorizationUri, new Uri(_bangumiOptions.CallbackUrl));
